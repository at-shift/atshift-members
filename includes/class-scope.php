...
final class Scope {
    const META='_asm_operator_scope';
    ...
    public static function groups($actor=null) {
        ...
        if(!Members::active($actor)||!user_can($actor,'asm_manage_members')||!self::ready() || !is_array($scope) || ($scope['mode']??'')!=='groups' || !is_array($scope['groups']??null))return [];
    ...
    public static function can_manage($target,$operation='view',$actor=null) {
        $actor=$actor??get_current_user_id();
        if(!user_can($actor,'asm_manage_members')||Members::blocked($actor))return false;
        if(user_can($actor,'manage_options') || !self::limited($actor))return true;
        $user=get_userdata($target);
        if(!$user || $user->roles!==['asm_member'] || (int)$target===$actor)return false;
    ...
    public static function can_assign($target,$before,$after) {
        ...
        if(!$u || $u->roles!==['asm_member'] || in_array(get_user_meta($target,'_asm_state',true),['withdrawing','withdrawn','provisioning'],true))return false;
    ...
    public static function content($id,$actor=null,$proposed=null) {
        ...
        if(isset(Posting::types()[$p->post_type])&&!in_array($p->post_type,['asm_post','asm_page','asm_notice'],true))return self::ready()&&(int)$p->post_author===$actor;
        if(!in_array($p->post_type,['asm_post','asm_page','asm_notice'],true))return false;
    ...
    public static function campaign($id,$actor=null) {
        $actor=$actor??get_current_user_id();$p=get_post($id);$plan=get_post_meta($id,'_asm_campaign',true);
        return $p&&$p->post_type==='asm_campaign'&&is_array($plan)&&self::delivery($plan['group']??'',$actor)
    ...
    public static function hooks() {
        ...
        add_filter('map_meta_cap',function($caps,$cap,$actor,$args){
            ...
            if(in_array($cap,['list_users','edit_users','create_users','delete_users','promote_users','publish_asm_notices','asm_import_members'],true))return ['do_not_allow'];
            return $caps;
        },PHP_INT_MAX,4);
        foreach(['asm_post','asm_page','asm_notice'] as $type)add_filter('rest_'.$type.'_query',function($args,$request){if($request->get_param('context')==='edit')$args['asm_scope_manage']=true;return $args;},10,2);
        add_filter('posts_where',function($where,$query){
            if((!is_admin()&&!$query->get('asm_scope_manage'))||!self::limited()||current_user_can('manage_options'))return $where;
            global $wpdb;$actor=get_current_user_id();$targets=self::target_ids();$groups=self::groups();
            $managed='0=1';if($targets&&$groups)$managed="{$wpdb->posts}.post_author IN (".implode(',',array_map('absint',$targets)).") AND {$wpdb->posts}.ID IN (SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key='_asm_audience' AND meta_value IN (".implode(',',array_map(function($k)use($wpdb){return $wpdb->prepare('%s',$k);},$groups)).'))';
            return $where." AND ({$wpdb->posts}.post_type NOT IN ('asm_post','asm_page','asm_notice') OR {$wpdb->posts}.post_author=".(int)$actor.' OR ('.$managed.'))';
        },PHP_INT_MAX,2);
