...
final class Media {
    ...
    public static function posts_sql($alias,$include_own=true) {
        ...
        foreach(Posting::types() as $name=>$type){
            ...
            if(Scope::limited()){
                if(!in_array($name,['atshme_post','atshme_page','atshme_notice'],true))continue;
                $groups=Scope::groups();$targets=Scope::target_ids();if(!$groups||!$targets)continue;
                $clause.=" AND $alias.post_author IN (".implode(',',array_map('absint',$targets)).") AND $alias.ID IN (SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key='_atshme_audience' AND meta_value IN (".implode(',',array_map(fn($s)=>$wpdb->prepare('%s',$s),$groups)).'))';
            }
            $others[]='('.$clause.')';
        }
    ...
    public static function attachments_sql($alias) {
        global $wpdb;
        return "($alias.post_author=".(int)get_current_user_id()." OR $alias.post_parent IN (SELECT atshme_parent.ID FROM {$wpdb->posts} atshme_parent WHERE ".self::posts_sql('atshme_parent',false).'))';
    }
    ...
    public static function permission() {
        return self::enabled()&&Members::reader()&&(!Scope::limited()||Scope::ready())?true:new \WP_Error('atshme_media_unavailable',__('This media action is unavailable.', 'atshift-members'),['status'=>403]);
    }
    ...
    public static function listing($r) {
        ...
        $private="EXISTS (SELECT 1 FROM {$wpdb->postmeta} pm WHERE pm.post_id=p.ID AND pm.meta_key='_atshme_private_file')";
    ...
    public static function upload($r) {
        ...
        if($mode==='members')$id=Files::save_upload($parent,$file);
            ...
            $_FILES['atshme_media_upload']=$file;
            try{$id=media_handle_upload('atshme_media_upload',$parent,[],['test_form'=>false]);}finally{unset($_FILES['atshme_media_upload']);}
        }
    ...
    public static function select($r) {
        ...
        try{
            ...
            global $wpdb;$copy=$wpdb->get_var($wpdb->prepare("SELECT p.ID FROM {$wpdb->posts} p JOIN {$wpdb->postmeta} m ON m.post_id=p.ID AND m.meta_key='_atshme_media_source' WHERE p.post_type='attachment' AND p.post_parent=%d AND p.post_author=%d AND m.meta_value=%s AND p.post_status<>'trash' LIMIT 1",$target,get_current_user_id(),(string)$id));
            ...
            $temp=wp_tempnam('atshme-copy');$out=$temp?fopen($temp,'wb'):false;
            ...
            try{
                ...
                update_post_meta($new,'_atshme_media_source',$id);return self::item($new);
            // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- Copy an authorized private stream through a WordPress temporary file; bounded streams are closed in finally.
    ...
    public static function assets($post=0,$library=false) {
        ...
        wp_enqueue_script('atshme-media',plugins_url('assets/media.js',$base),$deps,filemtime(dirname(__DIR__).'/assets/media.js'),true);
        wp_set_script_translations('atshme-media', 'atshift-members', dirname(__DIR__) . '/languages');
        wp_enqueue_style('wp-components');wp_enqueue_style('atshme-media',plugins_url('assets/media.css',$base),[],filemtime(dirname(__DIR__).'/assets/media.css'));
        wp_add_inline_script('atshme-media','window.atshmeMediaConfig='.wp_json_encode(['postId'=>(int)$post,'library'=>$library,'startUpload'=>($GLOBALS['pagenow']??'')==='media-new.php','maxSize'=>Files::max_upload_size(),'maxSizeLabel'=>size_format(Files::max_upload_size()),'accept'=>'.'.implode(',.',array_keys(Files::available_formats())),'rest'=>esc_url_raw(rest_url('atshift-members/v1/media'))]).';','before');
    }
    public static function library_page() {
        if(!self::enabled()||!Members::reader())wp_die(esc_html__('This media screen is unavailable.', 'atshift-members'),'',['response'=>403]);
        ...
        echo ('<div class="wrap">' . '<h1>' . esc_html__('Attach Files', 'atshift-members') . '</h1>' . '<div id="atshme-media-library">' . '</div>' . '</div>');
    }
    public static function redirect_library() {
        if(!self::member()||!self::enabled()||!self::can_upload())return;
        wp_safe_redirect(admin_url('upload.php?page=atshift-members-media'));exit;
    }
    public static function hooks() {
        ...
        add_action('admin_menu',function(){
            if(self::enabled()&&Members::reader()&&current_user_can('upload_files')&&self::can_upload())self::$library_hook=add_media_page(__('Attach Files', 'atshift-members'),__('Attach Files', 'atshift-members'),'upload_files','atshift-members-media',[self::class,'library_page']);
        });
        add_action('admin_enqueue_scripts',function($hook){
            $screen=get_current_screen();global $post;
            if(self::$library_hook!==''&&$hook===self::$library_hook){self::assets(0,true);return;}
            if($screen&&$screen->base==='post'&&$post&&self::enabled()&&self::target($post->ID))self::assets($post->ID);
        });
        add_action('load-upload.php',[self::class,'redirect_library']);add_action('load-media-new.php',[self::class,'redirect_library']);
        ...
        add_filter('posts_where',function($where,$q){
            if(!self::member()||(!is_admin()&&!$q->get('atshme_media_manage')))return $where;
            global $wpdb;$type=$q->get('post_type');
            if($type==='attachment')return $where.' AND '.self::attachments_sql($wpdb->posts);
            if($q->is_main_query()||$q->get('atshme_media_manage'))return $where.' AND '.self::posts_sql($wpdb->posts);
            return $where;
        },PHP_INT_MAX,2);
        add_action('init',function(){foreach(array_keys(Posting::types()) as $type)add_filter('rest_'.$type.'_query',function($args,$r){if($r['context']==='edit')$args['atshme_media_manage']=true;return $args;},10,2);},PHP_INT_MAX);
        add_filter('rest_attachment_query',function($args){$args['atshme_media_manage']=true;return $args;});
        ...
        add_action('wp_ajax_get-attachment',function(){check_ajax_referer('get-attachment');if(self::member()&&!self::can_select(absint(wp_unslash($_POST['id']??0))))wp_send_json_error(['message'=>__('You cannot manage this media item.', 'atshift-members')],403);},0);
