...
final class Admin {
    ...
    /** Keep configuration private even when an extension registers a broad capability. */
    public static function section_allowed($slug,$section) {
        if(!current_user_can($section['capability']??'manage_options'))return false;
        if(current_user_can('manage_options'))return true;
        if(!in_array($slug,['atshift-members','atshift-members-operations'],true))return false;
        return $slug==='atshift-members'?self::can_access_settings():current_user_can('atshme_use_workspace');
    }
    public static function can_access_settings() {
        if(!current_user_can('atshme_manage_members'))return false;
        if(current_user_can('manage_options'))return true;
        $user=wp_get_current_user();
        return in_array('atshme_operator',$user->roles,true)&&Members::active($user->ID)&&(!Scope::limited()||Scope::ready());
    }
    public function hooks() {
        ...
        add_action('admin_enqueue_scripts',function($hook){
            ...
            wp_enqueue_script('atshme-role-groups',plugins_url('assets/role-groups.js',dirname(__DIR__).'/atshift-members.php'),[],'0.1.6',true);
            wp_localize_script('atshme-role-groups','atshmeRoleGroups',['names'=>['administrator'=>__('Site Administrator', 'atshift-members'),'atshme_operator'=>__('Site Operator', 'atshift-members'),'atshme_member'=>__('User (Posting Permissions)', 'atshift-members')],'members'=>__('Membership Site Roles', 'atshift-members'),'standard'=>__('WordPress Standard Roles', 'atshift-members'),'other'=>__('Other Plugin Roles', 'atshift-members'),'help'=>__('Choose Site Operator for membership administration. Assign duties and management scope in Staff and Permissions.', 'atshift-members')]);
        });
        ...
        add_action('admin_menu',function(){
            ...
            add_menu_page('atshift Members','Members','atshme_manage_members','atshift-members',[$this,'page'],'dashicons-groups');
            foreach($this->sections() as $slug=>$section)if(self::section_allowed($slug,$section))add_submenu_page('atshift-members',$section['title'].' — Members',$section['title'],$section['capability'],$slug,[$this,'page']);
        ...
        add_action('admin_enqueue_scripts',function($hook){if(str_contains($hook,'atshift-members'))wp_enqueue_style('atshme-member-admin',plugins_url('assets/admin.css',dirname(__DIR__).'/atshift-members.php'),[],filemtime(dirname(__DIR__).'/assets/admin.css'));});
        add_action('admin_post_atshme_admin',[$this,'save']);
    ...
    private function form($action,$id='') {echo '<form'.($id?' id="'.esc_attr($id).'"':'').' method="post" action="'.esc_url(admin_url('admin-post.php')).'"><input type="hidden" name="action" value="atshme_admin"><input type="hidden" name="operation" value="'.esc_attr($action).'">';wp_nonce_field('atshme_admin');}
    public static function save_action($form,$title,$label,$description) {
        self::$save_actions[$form]=compact('title','label','description');
    }
    private static function member_settings_input() {
        $input=[];
        foreach(['state','state_before','member_role','role_before'] as $key){
            // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Called only after save() verifies atshme_admin; the copied scalar is unslashed and allowlisted below.
            $value=$_POST[$key]??null;
            if($value===null)continue;
            if(!is_string($value))return new \WP_Error('input',__('Check the submitted member settings.', 'atshift-members'));
            $input[$key]=sanitize_key(wp_unslash($value));
        }
        // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Called only after save() verifies atshme_admin; this structured field is unslashed and schema-validated below.
        $submitted_memberships=$_POST['memberships']??null;
        if($submitted_memberships!==null){
            if(!is_array($submitted_memberships))return new \WP_Error('input',__('Check the submitted classifications.', 'atshift-members'));
            $raw=wp_unslash($submitted_memberships);
            if(array_diff(array_keys($raw),['stamp','choices_stamp','groups'])||!is_string($raw['stamp']??null)||!is_string($raw['choices_stamp']??null)||!preg_match('/^[a-f0-9]{64}$/D',$raw['stamp'])||!preg_match('/^[a-f0-9]{64}$/D',$raw['choices_stamp']))return new \WP_Error('input',__('Check the submitted classifications.', 'atshift-members'));
            $groups=$raw['groups']??[];
            if(!is_array($groups)||count($groups)>200)return new \WP_Error('input',__('Check the submitted classifications.', 'atshift-members'));
            $clean=[];
            foreach($groups as $group){
                if(!is_string($group)||$group===''||strlen($group)>191)return new \WP_Error('input',__('Check the submitted classifications.', 'atshift-members'));
                $group=sanitize_text_field($group);
                if($group==='')return new \WP_Error('input',__('Check the submitted classifications.', 'atshift-members'));
                $clean[]=$group;
            }
            $input['memberships']=['stamp'=>$raw['stamp'],'choices_stamp'=>$raw['choices_stamp'],'groups'=>array_values(array_unique($clean))];
        }
        return $input;
    }
    private static function post_id($key) {
        // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Called only after save() verifies atshme_admin; the value is unslashed and must match the positive-decimal pattern below.
        $value=$_POST[$key]??'';
        if(!is_string($value))return 0;
        $value=wp_unslash($value);
        return preg_match('/^[1-9][0-9]*$/D',$value)?(int)$value:0;
    }
    private static function post_checked($key) {
        // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Called only after save() verifies atshme_admin; an enabled checkbox must be the exact literal 1.
        return is_string($_POST[$key]??null)&&wp_unslash($_POST[$key])==='1';
    }
    private static function query_id($key) {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Read-only admin selection; the value is unslashed and must match the positive-decimal pattern below.
        $value=$_GET[$key]??'';
        if(!is_string($value))return 0;
        $value=wp_unslash($value);
        return preg_match('/^[1-9][0-9]*$/D',$value)?(int)$value:0;
    ...
    public function save() {
        ...
        elseif ($op==='member_settings') {
            $input=self::member_settings_input();
            $result=is_wp_error($input)?$input:Members::save_settings($member_id,$input);
        }
        elseif ($op==='state') $result=Members::set_state($member_id,is_string($_POST['state']??null)?sanitize_key(wp_unslash($_POST['state'])):'');
        elseif ($op==='bulk_state') {
            // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- bulk_state() validates every array member as a positive decimal user ID before use.
            $users=wp_unslash($_POST['users']??[]);
            $bulk_state=is_string($_POST['bulk_state']??null)?sanitize_key(wp_unslash($_POST['bulk_state'])):'';
            $result=Members::bulk_state($users,$bulk_state);
            if(!is_wp_error($result)){
                set_transient('atshme_member_result_'.get_current_user_id(),$result,60);
        ...
        elseif ($op==='pages') {
            ...
            update_option('atshme_pages',$ids,false);
        } elseif ($op==='turnstile') {
            // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Structured input is validated by the operation-specific service after the shared nonce; mail fields are individually unslashed and sanitized below. Do not flatten arrays before schema validation.
            $result=Services::save_turnstile(wp_unslash($_POST['turnstile']??[]));
        } elseif ($op==='posting') {
            $stamp=is_string($_POST['posting_stamp']??null)?sanitize_text_field(wp_unslash($_POST['posting_stamp'])):'';
            // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Posting::save() validates the unslashed nested structure, type names, booleans, roles and classification keys against current server-side catalogs.
            $result=Posting::save(wp_unslash($_POST['posting']??[]),$stamp);
            if(!is_wp_error($result)){ $directory=Directory::options();$directory['hide_author']=self::post_checked('hide_author');update_option(Directory::OPTION,$directory,false);}
        } elseif ($op==='settings') {
            $raw_fields=is_string($_POST['fields']??null)?sanitize_text_field(wp_unslash($_POST['fields'])):implode(',',(array)Registration::option('fields',[]));
            $keys=preg_split('/[\s,]+/',$raw_fields,-1,PREG_SPLIT_NO_EMPTY);
            $selected=array_values(array_unique(array_map('sanitize_key',$keys)));
            if(!defined('ATSHME_PUBLIC_FIELDS')&&!Registration::api()){
            ...
            if(!is_wp_error($result))update_option('atshme_settings',['enabled'=>self::post_checked('enabled'),'approval'=>self::post_checked('approval'),'name_order'=>Registration::uses_profile_plugin()?Registration::option('name_order','given_first'):((is_string($_POST['name_order']??null)?sanitize_key(wp_unslash($_POST['name_order'])):'')==='family_first'?'family_first':'given_first'),'fields'=>(defined('ATSHME_PUBLIC_FIELDS')||Registration::api())?(array)Registration::option('fields',[]):$selected],false);
        } elseif ($op==='mail') {
            $templates=(array)get_option('atshme_mail_templates',[]);
            foreach (Mail::defaults() as $type=>$def) {
                ...
                if (!$subject || !$body || strlen($subject)>300 || strlen($body)>20000 || (in_array($type,['verify','reset','email_verify'],true) && strpos($body,'{link}')===false)) {$result=new \WP_Error('template',__('Check the subject, body, and required {link} placeholder.', 'atshift-members'));break;}
            ...
            if (!is_wp_error($result)) {update_option('atshme_mail_templates',$templates,false);update_option('atshme_sender_name',sanitize_text_field(wp_unslash($_POST['sender']??'')),false);update_option('atshme_mail_signature',sanitize_textarea_field(wp_unslash($signature)),false);}
        } else $result=new \WP_Error('operation',__('This settings operation is unavailable.', 'atshift-members'));
        ...
        if(in_array($op,['state','notice','member_settings'],true)){$url=add_query_arg('member_id',$member_id,self::url());}else $url=self::url($section);
        if($op==='member_settings'&&!Scope::can_manage($member_id,'view'))$url=self::url();
    ...
    public static function source($guide,$description) {
        echo '<p class="description">'.wp_kses_post(self::product_copy(esc_html($description))).'</p><p class="atshme-source"><span class="atshme-source-label">'.wp_kses_post(self::product_copy(esc_html($guide))).'</span></p>';
    }
    ...
    public static function integration_group($plugins) {
        ...
        $names=implode('<span class="atshme-integration-join">'.esc_html__(' and ', 'atshift-members').'</span>',$badges);
        /* translators: %s: Plugin name badges. */
        $message=esc_html__('Functionality extended by %s', 'atshift-members');
        echo '<span class="atshme-integration-status">'.wp_kses_post(sprintf($message,$names)).'</span>';
    }
    // Shown only when the profile plugin reports the per-field exclusion setting (atshift User Profile Fields 1.3+).
    public static function profile_field_exclusion_notice() {
        $api=Registration::api();
        if(empty($api['owner_form']['member_form_exclusion']))return;
        echo '<p class="description atshme-profile-field-exclusion">';self::plugin_badge('atshift User Profile Fields');
        echo ' '.esc_html__('To keep a field out of registration and account editing forms, open the field in atshift User Profile Fields and turn on "Hide from member registration and account forms" under Member Forms.', 'atshift-members').'</p>';
    }
    ...
    public static function plugin_badge($name,$label='') {
        ...
        echo '<span class="atshme-integration-badge atshme-integration-badge-'.esc_attr($colors[$name]??'neutral').'">'.esc_html($labels[$name]??$name).'</span>';
    }
    public static function integration_description($context) {
        ...
        if(!$rows)return;
        echo '<p>'.esc_html__('The following integrations are in use.', 'atshift-members').'</p><ul class="ul-disc atshme-integration-description">';
    ...
    public static function available_integration($context) {
        ...
        if(!$plugins)return;
        echo '<p class="atshme-integration-description atshme-integration-available">'.wp_kses_post(sprintf(esc_html($messages[$context]),...array_map([self::class,'badge_html'],$plugins))).'</p>';
    }
    public static function passkey_tip() {
        /* translators: %s: Freeform Login plugin badge. */
        echo '<p class="atshme-integration-description">'.wp_kses_post(sprintf(esc_html__('Install and activate %s to enable passkeys on supported environments without additional site setup. After signing up, members can register and manage passkeys on the Edit Account Information page.', 'atshift-members'),self::badge_html('atshift Freeform Login'))).'</p>';
    }
    ...
    public static function product_copy($html) {
        ...
        foreach(wp_html_split($html) as $part){
            if(str_starts_with($part,'<')){
                if(preg_match('/^<span\b/i',$part)&&($skip||str_contains($part,'atshme-integration-badge')))$skip++;
    ...
    private function sections() {
        $sections=[
            'atshift-members'=>['title'=>__('Member Management', 'atshift-members'),'description'=>'','capability'=>'atshme_manage_members','render'=>[$this,'members_page']],
    ...
    private static function navigation_groups() {
        ...
        return [
            ['title'=>__('1. Set Up Member Pages', 'atshift-members'),'pages'=>['atshift-members-pages','atshift-members-directory','atshift-members-mail']],
            ...
            ['title'=>__('3. Pro Add-on Features', 'atshift-members'),'pages'=>['atshift-members-license','atshift-members-authoring','atshift-members-permissions','atshift-members-workflow','atshift-members-operations']],
    ...
    public function page() {
        ...
        $sections=$this->sections();$slug=is_string($_GET['page']??null)?sanitize_key(wp_unslash($_GET['page'])):'atshift-members';$section=$sections[$slug]??null;
        if(!$section||!self::section_allowed($slug,$section))wp_die(esc_html__('This page is unavailable.', 'atshift-members'),'',['response'=>403]);
        Screens::private_headers();
        $member_id=self::query_id('member_id');
        echo '<div class="wrap atshme-settings'.($slug==='atshift-members'&&!$member_id?' atshme-members-overview':'').'"><p class="atshme-kicker">atshift Members'.(self::pro_active()?' <span class="atshme-pro-badge">PRO</span>':'').('</p>' . '<nav class="atshme-settings-nav" aria-label="' . esc_attr__('atshift Members Settings', 'atshift-members') . '">');
        ...
        foreach($groups as $group){
            $links='';foreach($group['pages'] as $key){$item=$remaining[$key]??null;unset($remaining[$key]);if($item&&self::section_allowed($key,$item))$links.='<a'.(in_array($key,['atshift-members-license','atshift-members-authoring','atshift-members-permissions','atshift-members-workflow','atshift-members-operations'],true)?' class="atshme-nav-members-pro"':'').' href="'.esc_url(self::url($key)).'" '.($key===$slug?'aria-current="page"':'').'>'.esc_html($item['title']).'</a>';}
            if($links!=='')echo '<div class="atshme-nav-group'.($group['title']===''?' atshme-nav-group-unlabelled':'').'">'.($group['title']!==''?'<p class="atshme-nav-group-title">'.esc_html($group['title']).'</p>':'').'<div class="atshme-nav-group-links">'.wp_kses_post($links).'</div></div>';
        }
        echo '</nav><div class="atshme-settings-heading"><h1>'.esc_html($section['title']).'</h1>';
        self::integration_badges($slug);
        echo '</div><hr class="wp-header-end">';
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Read-only success flag set by a nonce-protected save handler; only the exact literal 1 is accepted.
        ...
        self::$save_actions=[];
        ob_start();if(!empty($section['description']))echo '<p class="atshme-intro">'.esc_html($section['description']).'</p>';call_user_func($section['render']);if(current_user_can('manage_options')&&in_array($slug,['atshift-members-posting','atshift-members-mail'],true))$this->pro_tip($slug);$body=ob_get_clean();
        if($slug==='atshift-members'&&$member_id)echo '<p class="atshme-member-back"><a href="'.esc_url(self::url()).('">' . esc_html__('← Back to members', 'atshift-members') . '</a>' . '</p>');
        ...
        echo '<div class="atshme-settings-layout'.(self::$save_actions?' atshme-has-save-sidebar':'').'"><div class="atshme-settings-main">'.$body.'</div>';
        if(self::$save_actions){
            echo ('<aside class="atshme-save-sidebar" aria-label="' . esc_attr__('Save Settings', 'atshift-members') . '">');
            foreach(self::$save_actions as $form=>$action)echo '<section class="postbox atshme-save-box" aria-labelledby="'.esc_attr($form).'-title"><div class="postbox-header"><h2 id="'.esc_attr($form).'-title">'.esc_html($action['title']).'</h2></div><div class="inside"><p>'.esc_html($action['description']).'</p></div><div class="atshme-save-footer"><button type="submit" class="button button-primary" form="'.esc_attr($form).'">'.esc_html($action['label']).'</button></div></section>';
            echo '</aside>';
        }
        echo '</div></div>';
    }
    ...
    public function plugins_page() {
        ob_start();
        $members_pro_active=function_exists('atshift_members_pro_features_available')&&atshift_members_pro_features_available();
        $members_pro_url=$members_pro_active?'https://plugins.at-shift.net/members/':'https://plugins.at-shift.net/members/price/#plans';
        echo '<section class="atshme-settings-card" id="atshme-related-members-pro"><h2 class="atshme-product-heading">';self::plugin_badge('atshift Members Pro');echo ('</h2>' . '<p>' . esc_html__('Add tools for reviewing posts, sending announcements, and sharing responsibilities among staff.', 'atshift-members') . '</p>' . '<ul class="atshme-benefits">');
        ...
        echo ('<li>' . '<strong>' . esc_html__('Staff Responsibilities', 'atshift-members') . '</strong>' . '<br>' . esc_html__('Assign post approvals, announcement delivery, and CSV member onboarding to individual staff members.', 'atshift-members') . '</li>');
    ...
    public function posting_page() {
        ...
        $this->form('posting','atshme-save-posting');
        echo '<input type="hidden" name="posting_stamp" value="'.esc_attr(Posting::stamp()).('">' . '<section class="atshme-settings-card">' . '<h2>' . esc_html__('Permissions by Post Type', 'atshift-members') . '</h2>' . '<p>' . esc_html__('Choose which members can create content for each post type registered on this site. All Members means members whose status is Active. By default, only standard WordPress posts are available to all members.', 'atshift-members') . '</p>' . '<p class="description">' . esc_html__('Members with permission can create, edit, publish, and delete their own content. They gain no permission to edit other people\'s content. These settings do not change who can read content. Published standard WordPress posts remain visible to the public.', 'atshift-members') . '</p>');
        foreach(Posting::types() as $name=>$type){
            $rule=Posting::rule($name);$mode=$rule['mode']??'none';$selected=is_array($rule['groups']??null)?$rule['groups']:[];
            echo '<div class="atshme-posting-rule"><h3>'.esc_html($type->label).' <code>'.esc_html($name).'</code></h3><p><label for="atshme-posting-'.esc_attr($name).('">' . esc_html__('Members Allowed to Post', 'atshift-members') . '</label>' . ' ' . '<select id="atshme-posting-').esc_attr($name).'" name="posting['.esc_attr($name).'][mode]" data-atshme-posting-mode><option value="all" '.selected($mode,'all',false).('>' . esc_html__('All Members', 'atshift-members') . '</option>' . '<option value="none" ').selected($mode,'none',false).('>' . esc_html__('No Members', 'atshift-members') . '</option>');
            ...
            echo '</select></p>';
            if($available){
                echo '<details class="atshme-disclosure atshme-posting-groups" '.($mode==='groups'?'open':'').('>' . '<summary>' . esc_html__('Allowed User Categories', 'atshift-members') . '</summary>' . '<p class="description">' . esc_html__('If you select multiple categories, membership in any one of them is sufficient.', 'atshift-members') . '</p>' . '<div class="atshme-choice-list">');
    ...
    public function members_page() {
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin display/filter parameters; mutation handlers separately require nonce and object capability checks.
        if(is_string($_GET['state']??null))echo '<input type="hidden" name="state" value="'.esc_attr(sanitize_key(wp_unslash($_GET['state']))).'">';
        $table->search_box(__('Search Members', 'atshift-members'),'atshme-members');echo '</form>';$table->views();
        $this->form('bulk_state','atshme-member-bulk');$table->display();echo '</form>';
    }
    private function member_detail($id) {
        ...
        $this->form('member_settings','atshme-member-state');
        echo '<input type="hidden" name="user_id" value="'.(int)$id.'">';
        echo '<section class="atshme-settings-card atshme-member-summary"><header class="atshme-member-identity"><h2 class="atshme-member-name">'.esc_html($user->display_name).'</h2><p class="atshme-member-email">'.esc_html($user->user_email).'</p></header>';
        $state=get_user_meta($id,'_atshme_state',true);
        ...
        $handoff=Members::handoff_error($id);if(is_wp_error($handoff))echo '<p class="atshme-warning">'.esc_html($handoff->get_error_message()).'</p>';
        ...
        echo ('<div class="atshme-member-status">' . '<h3>' . esc_html__('Member Status', 'atshift-members') . '</h3>' . '<span class="atshme-status-badge atshme-status-badge-').esc_attr($state_badge[1]).'">'.esc_html($state_badge[0]).'</span></div>';
        if(!is_wp_error(Members::state_error($id))){
            echo '<input type="hidden" name="state_before" value="'.esc_attr($state).('">' . '<div class="atshme-member-state-controls">' . '<label for="atshme-member-state-select">' . esc_html__('Change status:', 'atshift-members') . '</label>' . '<select id="atshme-member-state-select" name="state" required>');
            ...
            echo ('</select>' . '</div>' . '<div class="atshme-pro-tip atshme-warning atshme-posting-guidance">' . '<p>' . esc_html__('Saving a status change sends the member a notification email and ends any active login sessions.', 'atshift-members') . '</p>' . '</div>');
            self::save_action('atshme-member-state',__('Member Information', 'atshift-members'),__('Save Changes', 'atshift-members'),__('Save this member\'s status.', 'atshift-members'));
        }
        if(!is_wp_error(Members::role_error($id))){
            $role=$user->roles[0];
            echo ('<div class="atshme-member-role-section">' . '<h3>' . esc_html__('Member Role', 'atshift-members') . '</h3>' . '<input type="hidden" name="role_before" value="').esc_attr($role).('">' . '<div class="atshme-member-state-controls">' . '<label for="atshme-member-role-select">' . esc_html__('Change role:', 'atshift-members') . '</label>' . '<select id="atshme-member-role-select" name="member_role">');
            foreach(['atshme_member'=>__('Member (Posting Permissions)', 'atshift-members'),'atshme_operator'=>__('Site Operator', 'atshift-members')] as $value=>$label)echo '<option value="'.esc_attr($value).'" '.selected($role,$value,false).'>'.esc_html($label).'</option>';
            ...
            self::save_action('atshme-member-state',__('Member Information', 'atshift-members'),__('Save Changes', 'atshift-members'),__('Save the member status and role changes made on this screen.', 'atshift-members'));
        }
        echo ('<div class="atshme-member-posting-section">' . '<div class="atshme-member-posting">' . '<h3>' . esc_html__('Available Post Types', 'atshift-members') . '</h3>');
        $count=0;$color=0;
        foreach(Posting::types() as $type){
            ...
            echo '<span class="atshme-posting-badge atshme-posting-badge-'.(int)$tone.'">'.esc_html($label).'</span>';
            $count++;
        }
        if(!$count)echo ('<span>' . esc_html__('No posting permissions.', 'atshift-members') . '</span>');
        echo '</div><div class="atshme-pro-tip atshme-warning atshme-posting-guidance"><p>'.esc_html(current_user_can('manage_options')?__('Change the allowed post types in Posting Settings.', 'atshift-members'):__('Ask a site administrator to change the allowed post types.', 'atshift-members')).'</p></div></div></section>';
    ...
    private function member_integration_guide() {
        echo ('<section class="atshme-settings-card atshme-member-integration-guide">' . '<h2>' . esc_html__('User Categories and Posting Permissions', 'atshift-members') . '</h2>');
        if(Audience::api())self::integration_description('posting');else self::available_integration('posting');
        if(Scope::ready())self::integration_description('categories');else self::available_integration('categories');
        echo '</section>';
    }
    public function pages_page() {
        echo ('<section class="atshme-settings-card">' . '<h2>' . esc_html__('Member Pages', 'atshift-members') . '</h2>');
        ...
        foreach(Screens::definitions() as $key=>$def){echo '<details class="atshme-disclosure"><summary>'.esc_html($def[0]).'</summary><p>'.esc_html($descriptions[$key]??__('A page for member account procedures.', 'atshift-members')).('</p>' . '<p>' . esc_html__('To create a page yourself, paste this shortcode into its content: ', 'atshift-members') . '<code>' . '[').esc_html($def[1]).']</code></p>';
            ...
            if(in_array($key,['register','edit','account'],true)){
                ...
                if(!Registration::uses_profile_plugin()||!Registration::passkey_integration_available())echo ('<h3 class="atshme-extension-heading">' . esc_html__('Extend Your Site Further', 'atshift-members') . '</h3>');
        ...
        $this->form('pages','atshme-save-pages');echo '</form></section>';self::save_action('atshme-save-pages',__('Member Page Setup', 'atshift-members'),__('Create Member Pages', 'atshift-members'),__('Create all missing member pages. Existing pages are left unchanged.', 'atshift-members'));
        Dashboard::home_settings();
        $this->files_page();
    }
    public function registration_page() {
        $this->form('settings','atshme-save-settings');echo ('<section class="atshme-settings-card">' . '<h2>' . esc_html__('Accepting Member Registrations', 'atshift-members') . '</h2>');
        ...
        echo ('<section class="atshme-settings-card" id="atshme-profile-form-settings">' . '<h2>' . esc_html__('Registration and Editing Forms', 'atshift-members') . '</h2>');
        ...
        if(Registration::uses_profile_plugin()){
            ...
            if($configured_fields){
                ...
                foreach($configured_fields as $field){
                    ...
                    if(!empty($field['status_label']))echo ' <span class="atshme-field-status atshme-field-status-'.esc_attr($field['status']).'">'.esc_html($field['status_label']).'</span>';
                    echo '</li>';
                }
                ...
                self::profile_field_exclusion_notice();
        ...
        echo '<details class="atshme-disclosure"'.(Registration::uses_profile_plugin()?' open':'').('>' . '<summary>' . esc_html__('Add or Change Fields', 'atshift-members') . '</summary>');
        if(Registration::api()){
            echo ('<p>' . esc_html__('The profile layout, field order, and conditional rules are used automatically. You do not need to select fields again or grant additional front-end display or self-editing permissions here.', 'atshift-members') . '</p>');
            self::profile_field_exclusion_notice();
            echo ('<p>' . '<a class="button" href="').esc_url(admin_url('admin.php?page=atshift-user-profile-fields')).('">' . esc_html__('Edit Profile Fields in atshift User Profile Fields', 'atshift-members') . '</a>' . '</p>');
        } else {
        ...
        /* translators: Numbered placeholders contain field identifiers, plugin names, paths, or counts. */
            if(defined('ATSHME_PUBLIC_FIELDS'))echo ('<p>' . sprintf(esc_html__('The fields are specified by %s in the WordPress configuration file (wp-config.php).', 'atshift-members'),'<code>ATSHME_PUBLIC_FIELDS</code>') . '</p>');
        /* translators: Numbered placeholders contain field identifiers, plugin names, paths, or counts. */
        ...
        if(!Registration::uses_profile_plugin()||!Registration::passkey_integration_available())echo ('<hr style="margin:20px 0;border:0;border-top:1px solid #dcdcde">' . '<h3 class="atshme-extension-heading">' . esc_html__('Extend Your Site Further', 'atshift-members') . '</h3>');
        if(!Registration::uses_profile_plugin())self::available_integration('profile');
        ...
        echo '</section>';
        echo '</form>';self::save_action('atshme-save-settings',__('Registration Settings', 'atshift-members'),__('Save Registration Settings', 'atshift-members'),__('Save registration availability, approval requirements, and profile field settings.', 'atshift-members'));
        $this->turnstile_form();
        echo ('<aside class="atshme-pro-tip atshme-warning" aria-labelledby="atshme-cache-warning-title">' . '<h2 id="atshme-cache-warning-title">' . esc_html__('Check Cache Settings Before Launch', 'atshift-members') . '</h2>');
    ...
    public function mail_page() {
        $this->form('mail','atshme-save-mail');echo ('<section class="atshme-settings-card">' . '<h2>' . esc_html__('Sender Information', 'atshift-members') . '</h2>');
        ...
        echo ('<p>' . '<label>' . esc_html__('Sender Name ', 'atshift-members') . '<input name="sender" value="').esc_attr(get_option('atshme_sender_name','')).('">' . '</label>' . '</p>' . '<p class="description">' . esc_html__('Leave blank to use the WordPress sender name.', 'atshift-members') . '</p>');
        /* translators: Numbered placeholders contain field identifiers, plugin names, paths, or counts. */
        echo ('<p>' . '<label for="atshme-mail-signature">' . esc_html__('Shared Signature', 'atshift-members') . '</label>' . '<textarea id="atshme-mail-signature" name="signature" class="large-text" rows="6" aria-describedby="atshme-mail-signature-help" placeholder="' . esc_attr__('{site} Office
Contact: info@example.com
https://example.com/', 'atshift-members') . '">').esc_textarea(get_option('atshme_mail_signature','')).('</textarea>' . '</p>' . '<p id="atshme-mail-signature-help" class="description">' .
        ...
        foreach(Mail::defaults() as $type=>$def){
            ...
            $t=$saved[$type]??$def;echo '<details class="atshme-disclosure"><summary>'.esc_html($def['label']).'</summary><p class="description">'.esc_html($help[$type]??__('Explain what happened and what the recipient should do next.', 'atshift-members')).('</p>' . '<p>' . '<label>' . esc_html__('Subject', 'atshift-members') . '<input class="large-text" name="templates[').esc_attr($type).'][subject]" value="'.esc_attr($t['subject']).('" required>' . '</label>' . '</p>' . '<p>' . '<label>' . esc_html__('Body', 'atshift-members') . '<textarea class="large-text" rows="5" name="templates[').esc_attr($type).'][body]" required>'.esc_textarea($t['body']).'</textarea></label></p></details>';}
        echo '</form>';self::save_action('atshme-save-mail',__('Email Settings', 'atshift-members'),__('Save Email Settings', 'atshift-members'),__('Save the sender name, shared signature, and all email subjects and bodies.', 'atshift-members'));
    }
    private function storage_form($kind,$config) {
        if(defined('ATSHME_PRIVATE_DIR')||defined('ATSHME_PUBLIC_ROOT')||defined('ATSHME_FILE_STORE')){
            return;
        }
        $this->form('file_storage','atshme-file-storage-'.$kind);
    ...
    public function files_page() {
        echo ('<details class="atshme-settings-card atshme-disclosure" id="atshme-file-storage">' . '<summary>' . esc_html__('Storage for Member Documents and Images', 'atshift-members') . '</summary>');
        ...
        echo ('<details class="atshme-disclosure">' . '<summary>' . esc_html__('Use the Web Server Running WordPress', 'atshift-members') . '</summary>');
        ...
        echo ('<div class="atshme-pro-tip atshme-warning">' . '<p>' . esc_html__('Files marked Public are saved to the standard WordPress media directory. Anyone who knows the file URL can open them directly, even if the post is restricted to members. Storage does not change automatically when a post\'s visibility changes.', 'atshift-members') . '</p>' . '</div>');
        if(Files::upload_available()){
            $store=defined('ATSHME_FILE_STORE')?ATSHME_FILE_STORE:'local';
            echo ('<p class="atshme-storage-current">' . '<strong>' . esc_html__('Current storage: ', 'atshift-members')).esc_html($store==='local'?($config['kind']==='network'?__('File Server / NAS (Mounted Share)', 'atshift-members'):__('WordPress Web Server (Private Directory)', 'atshift-members')):$store).'</strong>';
            if(defined('ATSHME_PRIVATE_DIR')||defined('ATSHME_PUBLIC_ROOT')||defined('ATSHME_FILE_STORE'))echo ('<br>' . esc_html__('Configured in wp-config.php', 'atshift-members'));
