...
final class Audience {
    ...
    public static function allows($id,$user) {
        ...
        if(metadata_exists('post',$id,'_atshme_publication_lock')&&!user_can($user,'manage_options'))return false;
    ...
    public static function sql($column) {
        ...
        return "$column NOT IN (SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key='_atshme_publication_lock') AND $column NOT IN (SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key='_atshme_audience'$allowed)";
    }
    public static function hooks() {
        ...
        add_action('pre_get_comments',function($q){$q->query_vars['cache_domain']='atshme_'.Store::digest(wp_json_encode([get_current_user_id(),Members::reader(),self::keys(get_current_user_id()),current_user_can('atshme_manage_members')]));});
        foreach(['added_user_meta','updated_user_meta','deleted_user_meta'] as $hook)add_action($hook,function($mid,$uid,$key){if($key==='_atshme_state'){wp_cache_set_comments_last_changed();wp_cache_set_posts_last_changed();}},10,3);
        foreach(['added_post_meta','updated_post_meta','deleted_post_meta'] as $hook)add_action($hook,function($mid,$pid,$key){if(in_array($key,[self::META,'_atshme_members_only','_atshme_publication_lock'],true)){wp_cache_set_comments_last_changed();wp_cache_set_posts_last_changed();}},10,3);
        ...
        add_filter('wp_sitemaps_posts_query_args',function($args){$args['meta_query']=['relation'=>'AND',$args['meta_query']??[],['key'=>self::META,'compare'=>'NOT EXISTS'],['key'=>'_atshme_publication_lock','compare'=>'NOT EXISTS']];return $args;},20);
